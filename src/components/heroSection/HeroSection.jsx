      ...
      <div className="relative h-[560px] overflow-hidden md:h-[620px] lg:h-[680px]">
        {SLIDES.map((item, index) => {
          ...
          return (
            <div
              key={item.title}
              className={`absolute inset-0 transition-opacity duration-1000 ${selected ? "opacity-100" : "opacity-0"
                }`}
          ...
          <div className="flex items-center gap-1.5">
            {SLIDES.map((_, i) => (
              <button
                ...
                onClick={() => setActive(i)}
                className={`h-2 cursor-pointer rounded-full transition-all duration-300 ${i === active ? "w-8 bg-orange-500" : "w-2 bg-white/40 hover:bg-white/60"
                  }`}
              />
            ))}
          </div>
